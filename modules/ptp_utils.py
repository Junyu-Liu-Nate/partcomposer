...
class AttentionControl(abc.ABC):
    ...
    #     '''
    #     The implementation only processed the second half of the attention map in the batch,
    #     which is designed for classifier-free guidance (CFG).
    #     '''
    ...
    def __call__(self, attn, is_cross: bool, place_in_unet: str, batch_size: int):
        # Process all attention maps
        attn = self.forward(attn, is_cross, place_in_unet, batch_size)
        self.cur_att_layer += 1
...
class AttentionStore(AttentionControl):
    ...
    @staticmethod
    def get_empty_store():
        # Dynamically generate keys based on the attention processors
        keys = [f"{place}_{attn_type}" for place in ["down", "mid", "up"] for attn_type in ["cross", "self"]]
    ...
    def forward(self, attn, is_cross: bool, place_in_unet: str, batch_size = 2):
        # print(f'self.step_store keys: {self.step_store.keys()}')
        key = f"{place_in_unet}_{'cross' if is_cross else 'self'}"
        if attn.shape[1] <= 32 ** 2:
            # Ensure batch dimension is correctly handled
            num_heads = attn.shape[0] // batch_size
            attn = attn.view(batch_size, num_heads, attn.shape[1], attn.shape[2])
            self.step_store[key].append(attn)
        # print(f'key: {key}, attn.shape: {attn.shape}')
        return attn
    ...
    def __init__(self):
        ...
        # print('Check check check initialized AttentionStore!')
        self.step_store = self.get_empty_store()
        self.attention_store = {}
