...
def parse_args(input_args=None):
    '''
    The arg parser argumments are grouped as follows:
        - Pre-trained models
        - Dataset
        - Training
        - Log and Save
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        help="Disgarded! A folder containing the training data of class images.",
    )
    parser.add_argument(
        "--class_prompt",
        type=str,
        default="a photo of a chair.",
    ...
    parser.add_argument(
        ...
        default=395,
        help=(
            "Disgarded! Minimal class images for prior preservation loss. If there are not enough images already present in"
    ...
    parser.add_argument(
        ...
        action="store_false",
        help="Flag to add prior preservation loss. Disgarded! May use for running BaS baseline",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Disgarded! The weight of prior preservation loss.",
    )
    parser.add_argument(
        "--prior_prob",
        type=float,
        default=0.1,
        help="Probability of using prior preservation img and apply loss.",
    )
    parser.add_argument(
        "--use_prior_data",
        action="store_true",
        default=False,
        help="Use prior preservation loss to improve img quality.",
    )
    parser.add_argument(
        "--prior_data_dir",
        type=str,
        default=None,
        help="Folder that contains class‑prior images (e.g. generic ‘chair’ photos). \
              Each image file inside the folder is used purely for prior‑preservation.",
    )
    parser.add_argument(
        "--lambda_prior_loss",
        type=float,
        default=1.0,
        help="Weight of the prior‑preservation MSE loss.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--apply_weighted_masked_loss",
        action="store_true",
        help="Use weighted masked loss instead of computing the loss all a combined big mask",
    )
    parser.add_argument(
        "--apply_max_pooling_on_diff_masks",
        action="store_true",
        help="Use max pooling to downsample diffusion mask from [512,512] to [64,64]",
    )
    parser.add_argument(
        "--apply_max_pooling_on_atten_masks",
        action="store_true",
        help="Use max pooling to downsample attention mask from [512,512] to [16,16]",
    )
    parser.add_argument(
        "--apply_bg_loss",
        action="store_true",
    ...
    parser.add_argument(
        "--concept_predictor_guide_inference",
        action="store_true",
        help="Use concept train_concept_predictor to guide the inference process.",
    )
    parser.add_argument(
        "--concept_pred_train_step_range",
        type=float,
        default=1.0,
        help="The range of training steps for the concept predictor. 0.5 indicates do concept predition for the later 50% of the steps",
    )
    parser.add_argument(
        "--use_time_weight",
        action="store_true",
        help="Use time weight for the concept predictor loss.",
    )
    parser.add_argument(
        "--apply_grad_norm",
        action="store_true",
        help="Apply grad norm on cls and seg loss in concept predictor.",
    )
    ...
    parser.add_argument(
        "--use_MuDI_inference",
        action="store_true",
        help="Indicator to use MuDI for inference - start with a mean shifted latent instead of a completely random one",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        "--log_masks",
        action="store_true",
        help="Indicator to log masks to wandb",
    )
    parser.add_argument(
        "--log_atten_maps",
        action="store_true",
        help="Indicator to log attention maps to wandb",
    )
    parser.add_argument(
        "--log_last_step_atten",
        action="store_true",
        help="Indicator to log last step attention maps to wandb",
    )
    parser.add_argument(
        "--log_inference_atten",
        action="store_true",
        help="Indicator to log inference attention maps to wandb",
    )
    parser.add_argument(
        "--log_concept_predictor",
        action="store_true",
        help="Indicator to log concept predictor outputs to wandb",
    )
